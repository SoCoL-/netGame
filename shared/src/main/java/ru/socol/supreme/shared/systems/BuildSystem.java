...
public class BuildSystem extends EntitySystem {
    ...
    private void updateBuilder(Entity builder) {
        ...
        if (distance > buildRange) {
            order.inRange = false;
            // Точка подхода считается либо один раз (пока hasApproachPoint
            // false — первый тик, когда стало ясно, что строитель ещё не
            // в радиусе), либо заново — если строитель уже "дошёл" до
            // старой точки (Pathfinding/MovementSystem сбросили
            // direction.moving на false), а до цели всё ещё дальше
            // buildRange. Второе условие — защита от редкого, но
            // реального зависания: если CollisionSystem (сосед или
            // соседнее здание) на долю юнита отодвинула строителя ровно в
            // момент прибытия, старая точка подхода может оказаться
            // физически недостижимой — без пересчёта строитель молча
            // стоял бы рядом с ней вечно, гоняясь за координатами, занять
            // которые ему не дают. Пересчитываем НЕ каждый тик (см. ниже,
            // почему) — только в этот единственный момент, когда движение
            // к прежней точке уже закончилось (moving стало false), а
            // строитель всё ещё не в радиусе; пока он реально идёт туда,
            // это условие не срабатывает.
            //
            // Пересчёт каждый тик ВО ВРЕМЯ движения был реальным багом:
            // направление "откуда идёт строитель" чуть-чуть меняется на
            // каждом шаге движения, и пересчитанная от него точка подхода
            // иногда попадала в СОСЕДНЮЮ клетку сетки A* по сравнению с
            ...
            if (!order.hasApproachPoint || !direction.moving) {
                // Целимся чуть ВНУТРЬ границы buildRange (отступ на
                // ARRIVE_THRESHOLD), а не точно на неё. MovementSystem при
                // прибытии "прилипает" ровно к запрошенным координатам
                // (см. её processEntity), но это ровно buildRange от цели
                // только математически: сама точка подхода получена через
                // nor()/scl() от текущего направления, а дистанция потом
                // меряется заново отдельным dst() в другую сторону —
                // из-за округления float результат мог разойтись на
                // исчезающе малую величину и оказаться чуть БОЛЬШЕ
                // buildRange. Строитель тогда навсегда замирал в
                // сотых долях юнита от цели, ни разу не проходя проверку
                // distance > buildRange выше, — то самое "строитель
                // подъезжает и не строит, будто встаёт чуть дальше, чем
                // радиус постройки". Отступ с большим запасом перекрывает
                // эту погрешность и остаётся безопасным: buildRadius у
                // строителя (210) на порядок больше и ARRIVE_THRESHOLD, и
                // самого раздутого здания, так что точка подхода не может
                // из-за этого попасть в заблокированную зону вокруг цели.
