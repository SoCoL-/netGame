 ...
 * юнитами, эта система её вовсе не касается. Этого одного исключения
 * недостаточно: сетка соседей (grid) в update() строится по ВСЕМ
 * unitsById, а не по entities этой Family, так что авиацию нужно
 * пропускать и там отдельно — иначе она сама не обрабатывалась бы (не
 * толкалась), но продолжала бы попадать наземным юнитам в grid.query как
...
public class CollisionSystem extends IteratingSystem {
    ...
    public void update(float deltaTime) {
        ...
        for (Entity other : unitsById.values()) {
            ...
            if (other.getComponent(AircraftComponent.class) != null) {
                // Авиация вообще не попадает в сетку — иначе, даже будучи
                // исключённой из Family этой системы (сама она никогда не
                // обрабатывается и не толкается), она всё равно находилась
                // бы наземными юнитами как сосед через grid.query ниже и
                // отталкивала бы их от себя, как препятствие — та самая
                // ошибка: самолёт "сталкивается" с наземными юнитами
                // односторонне, потому что летает поверх них, а не
                // участвует в наземной физике вовсе.
                continue;
            }
