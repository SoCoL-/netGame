 ...
 * достижении 0 система сама снимает этот компонент и добавляет тот,
 * который делает здание функциональным (сейчас — ResourceExtractorComponent
 * для здания добычи железа). На клиенте — то же самое, только для
 * отрисовки: пока компонент есть, RenderSystem рисует "стройку" (тусклый
 * цвет + прогресс-бар) вместо готового здания.
 */
