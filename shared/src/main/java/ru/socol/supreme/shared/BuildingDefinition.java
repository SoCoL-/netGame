 ...
 * ненулевое у шахты, станции и турели (см. их javadoc чуть ниже).
 *
 * TURRET — единственное здание, которое умеет атаковать: урон/дальность
 * атаки/скорострельность для него всё равно не тут, а в UnitDefinitions
 * по отдельному UnitType.TURRET (см. javadoc GameServer.spawnBuilding,
 * почему это две роли одной и той же сущности) — в BuildingDefinition
 * таких полей нет и не должно быть, здесь только "здание" половина данных
 * (размер/здоровье/стоимость/постройка/обзор).
 */
